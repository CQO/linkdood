...
function run (command, color, name) {
  ...
  child.stdout.on('data', data => {
    ...
    if (/Compiled successfully/g.test(data.toString().trim().replace(/\n/g, '\n' + repeat(' ', command.length + 2))) && !isElectronOpen) {
      ...
      run('cross-env NODE_ENV=development electron app/main/index.dev.js', BLUE, 'electron')
      isElectronOpen = true
    }
  })
...
run(`webpack-dev-server --hot --colors --config webpack.renderer.config.js --port ${config.port} --content-base builds/dist`, YELLOW, 'webpack')
